# -*- coding: utf-8 -*-
"""
Created on Sun Mar 13 19:25:40 2022
...
def raspi_interp_preprocessing(raspi_path, sensor_num, save_path):
    ...
    sensor_data = pd.read_csv(sensor_csv, sep=" |,", header=None)
    sensor_data = sensor_data.astype(float)
    try:
        sensor_data[[12]]
        sensor_data = sensor_data[[2, 3, int(sensor_num)+4]]
        pass
    except KeyError:
        if sensor_data[[2]].isnull().sum()[2] > 100:
            sensor_data = sensor_data[[0, 1, int(sensor_num)+2]]
            pass
        else:
            sensor_data = sensor_data[[1, 2, int(sensor_num)+3]]
            pass
    else:
        sensor_data = sensor_data[[2, 3, int(sensor_num)+4]]
        pass
...
def calib_interp(instron_path, instron_sync_time, sensor_path, save_path):
    ...
    force_synced = pd.DataFrame(np.interp(sensor_synced["time"],
                                          instron_synced["Time"],
                                          instron_synced["Force"]),
                                columns=["force"])

    calib_synced = pd.DataFrame(sensor_synced)
    calib_synced = pd.concat([calib_synced, force_synced], axis=1)
    calib_synced.columns = ["sync", "time", "vout", "force"]
