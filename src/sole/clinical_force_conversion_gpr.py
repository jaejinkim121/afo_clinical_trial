...
base_path = "C:/Users/minhee/OneDrive - SNU/AFO_exp_data/*"
base_name_path = "C:/Users/minhee/OneDrive - SNU/AFO_exp_data/"

file_list = glob.glob(base_path)
file_name_list = os.listdir(base_name_path)
exp_list = [file for (file, name) in zip(file_list, file_name_list) if name.startswith("exp")==1]
exp_name_list = [name for (file, name) in zip(file_list, file_name_list) if name.startswith("exp")==1]
exp_list = [file.replace('\\','/') for file in exp_list]

for (RH_list, RH_name) in zip(exp_list, exp_name_list):
    
    df_train = pd.DataFrame(columns=["case_num", "RH", "folder_name", "sample_num","time", "pred_time","explained_variance_score", "mean_absolute_error", "mean_squared_error", "RMSE_KPA", "R2"])
    df_test = pd.DataFrame(columns=["case_num", "RH", "stance_or_walk", "stance_or_walk_num", "sensor_dir", "sensor_num", "sample_num","pred_time"])
    
    RH_path = str(RH_list)+"/*"
    RH_name_path = str(RH_list)+"/"
    
    RH_int_list = glob.glob(RH_path)
    RH_int_list = [file.replace('\\','/') for file in RH_int_list]
    RH_int_name_list = os.listdir(RH_name_path)
    
    RH_num = [file for file in RH_int_name_list if file.startswith("RH")==1]
    RH_num = RH_num[0][3:]
    
    if not os.path.exists(str(RH_name_path)+"Calibration"):
    ...
    calib_path = RH_name_path+calib_name+"/*"
    calib_name_path = RH_name_path+calib_name+"/"
    
    calib_list_ = glob.glob(calib_path)
    calib_name_list_ = os.listdir(calib_name_path)
    calib_test_list = [file for (file, name) in zip(calib_list_, calib_name_list_) if name.endswith("txt")==0]
    calib_test_list = [file.replace('\\','/') for file in calib_test_list]
    calib_name_list = [name for (file, name) in zip(calib_list_, calib_name_list_) if name.endswith("txt")==0]
    
    for (calib_num_path, calib_num_name) in zip(calib_test_list, calib_name_list):
        ...
        for case in np.arange(CASE,CASE+1,1):
            ...
            if calib_name == "Calibration":
                ...
                training_path = str(calib_num_path)+"/N_data_test.csv"
                ...
                training_path1 = str(calib_num_path)+"/N_data_test.csv"
                training_path2 = str(RH_name_path)+"Calibration_pre/"+str(calib_num_name)+"/N_data_test.csv"
    ...
    df_train.to_csv(RH_name_path+"df_train_N_total.csv",sep=",",header=True, index=False)
