...
import statistics
from calib_functions import folder_path_name, clinical_force_data_reading
from calib_functions import sync_for_clinical_data, synced_data_plot
from calib_functions import COP_data_read_plot, GRF_data_read_plot
from calib_functions import sensor_stance_data_sync_plot_save
from calib_functions import sensor_walk_data_sync_plot_save
from calib_functions import force_sync_indexing, sensor_re_sync, force_interp
from calib_functions import N_data_preprocessing
...
for (RH_list, RH_name) in zip(exp_list, exp_name_list):
    ...
    for (folder_list, folder_name) in zip(RH_int_list, RH_int_name_list):
        ...
        ##############################################################
        # force plate data
        if folder_name.startswith("RH") == 1:
            ##########################################################
            ##########################################################
            # stance data
            try:
                stance_data_name_list = os.listdir(str(folder_list) +
                                                   "/stance_data/")
            except FileNotFoundError:
                pass
            else:
                (stance_data_list, stance_data_name_list) = folder_path_name(
                    str(folder_list) + "/stance_data/", "end", "anc", 1)

                for (stance_file, stance_name) in zip(stance_data_list,
                                                      stance_data_name_list):
                    # data read, raw data plot
                    force1_data, force2_data, time_data, \
                        sync_data = clinical_force_data_reading(
                            stance_file, stance_name[:-4], str(RH_list) +
                            "/Graph/raw", str(RH_num))
                    #########################################################
                    # stance force synced data plotting
                    if (folder_name[-2:] == "03"):
                    ...
                    synced_data_plot(
                        ...
                        synced_index, RH_num, stance_name[:-4],
                        str(RH_list)+"/Graph/stance_%s" % (stance_name[-6:-4]),
                        str(folder_list) + "/stance_data/stance_%s" % (
                            stance_name[-6:-4]))

                    df_sync_force = df_sync_force.append({
                        "RH_num": "RH-" + str(RH_num),
                        "stance or walk": "stance",
                        "number": str(stance_name[-6:-4]),
                    ...
                    #########################################################
                    # stance COP data
                    stance_COP_file = str(stance_file[:-4])+".forces"

                    COP_data_read_plot(
                        stance_COP_file, synced, synced_index, time_data,
                        stance_name[:-4], RH_num, str(RH_list) +
                        "/Graph/stance_%s" % (stance_name[-6:-4]),
                        str(folder_list) + "/stance_data/stance_%s" % (
                            stance_name[-6:-4]))
            ...
            #################################################################
            # walk data
            walk_path = str(folder_list) + "/walk_data/"

            try:
                walk_data_name_list = os.listdir(walk_path)
            except FileNotFoundError:
                pass
            else:
                ...
                for (walk_file, walk_name) in zip(walk_data_list,
                    ...
                    force1_data, force2_data, time_data, \
                        sync_data = clinical_force_data_reading(
                            walk_file, walk_name[:-4], str(RH_list) +
                            "/Graph/raw", str(RH_num))

                    ##########################################################
                    # walk force synced data plotting
                    if (folder_name[-2:] == "04") & any(walk_name[-6:-4] == s
                                                        for s in ["12", "17"]):
                        synced_index = sync_for_clinical_data(
                            sync_data, sync_condition="small")
                    else:
                    ...
                    synced_data_plot(
                        ...
                        str(RH_list)+"/Graph/walk_%s" % (walk_name[-6:-4]),
                        str(folder_list) + "/walk_data/walk_%s" % (
                            walk_name[-6:-4]))

                    df_sync_force = df_sync_force.append({
                        "RH_num": "RH-" + str(RH_num),
                        "stance or walk": "walk",
                        "number": str(walk_name[-6:-4]),
                        "index start": synced_index[0],
                        "index end": synced_index[-1],
                        "total end of index": time_data.index[-1],
                        "time start": time_data.loc[synced_index[0], "Name"],
                    ...
                    COP_data_read_plot(
                        ...
                        walk_name[:-4], RH_num, str(RH_list) + "/Graph/walk_%s"
                        % (walk_name[-6:-4]), str(folder_list) +
                        "/walk_data/walk_%s" % (walk_name[-6:-4]))
                ...
                for (GRF_file, GRF_name) in zip(walk_GRF_data_list,
                    ...
                    GRF_data_read_plot(GRF_file, GRF_name[:-4], RH_num,
                                       str(RH_list)+"/Graph/walk_%s"
                                       % (GRF_name[-6:-4]), str(folder_list) +
                                       "/walk_data/walk_%s" % (
                                            GRF_name[-6:-4]))

    df_sync_force.to_csv(str(RH_list) + "/df_sync_force.csv", sep=",",
...
for (RH_list, RH_name) in zip(exp_list, exp_name_list):
    ...
    for (folder_list, folder_name) in zip(RH_int_list, RH_int_name_list):
        ...
        if folder_name == "RasPi":
            ...
            ##########################################################
            # stance data
            stance_path = str(folder_list) + "/stance_data/"

            try:
                stance_data_name_list = os.listdir(stance_path)
            except FileNotFoundError:
                pass
            else:
                (L_stance_data_list,
                 L_stance_name_list) = folder_path_name(
                      stance_path, "start", "L", 1)
                (R_stance_data_list, R_stance_name_list) = folder_path_name(
                    stance_path, "start", "R", 1)

                df_sync = sensor_stance_data_sync_plot_save(
                    L_stance_data_list, L_stance_name_list, R_stance_data_list,
                    R_stance_name_list, df_sync, RH_num, stance_num,
                    str(RH_list) + "/Graph",
                    str(RH_list) + "/Raspi/stance_data")
            ...
            try:
                ...
                df_sync = sensor_walk_data_sync_plot_save(
                    ...
                    str(RH_list) + "/Graph",
                    str(RH_list) + "/Raspi/walk_data")

    df_sync.to_csv(str(RH_list) + "/df_sync.csv", sep=",",
                   index=False, header=True)
    ...
    for (folder_list, folder_name) in zip(RH_int_list, RH_int_name_list):
        ...
        if folder_name == "RasPi":
            ...
            if RH_num == "02":
                ...
                # stance data
                (stance_data_list, stance_data_name_list) = folder_path_name(
                    str(folder_list) + "/stance_data/", "start", "stance", 1)

                for (stance_file, stance_name) in zip(stance_data_list,
                                                      stance_data_name_list):

                    stance_num = stance_name[7:]
                    if (len(str(stance_num)) > 1) & (
                            str(stance_num)[0] == "0"):
                        stance_num = str(stance_num)[1]
                    else:
                        pass

                    # reference index, time from force plate data
                    ref_start_index_S, ref_end_index_S, \
                        true_start_index_S, ref_start_time_S, \
                        ref_end_time_S, ref_del_time_S = force_sync_indexing(
                                df_sync_force, RH_num, "stance", stance_num)
                    ######################################################
                    # L data
                    (Lstance_data_list,
                     Lstance_data_name_list) = folder_path_name(str(
                          stance_file) + "/L/", "start", "RH", 1)

                    for (Lstance_final_file, Lstance_final_name) in zip(
                            Lstance_data_list, Lstance_data_name_list):

                        sensor_num = str(Lstance_final_name[-5:-4])
                        Lsensor_stance_file = Lstance_final_file

                        # re sync
                        sensor_re_sync(Lsensor_stance_file, sensor_num, str(
                          stance_file) + "/L/", ref_start_index_S,
                            ref_end_index_S, true_start_index_S,
                            ref_start_time_S, ref_end_time_S,
                            ref_del_time_S)

                    ######################################################
                    # R data
                    (Rstance_data_list,
                     Rstance_data_name_list) = folder_path_name(str(
                          stance_file) + "/R/", "start", "RH", 1)

                    for (Rstance_final_file, Rstance_final_name) in zip(
                            Rstance_data_list, Rstance_data_name_list):

                        sensor_num = str(Rstance_final_name[-5:-4])
                        Rsensor_stance_file = Rstance_final_file

                        # re sync
                        sensor_re_sync(Rsensor_stance_file, sensor_num, str(
                          stance_file) + "/R/", ref_start_index_S,
                            ref_end_index_S, true_start_index_S,
                            ref_start_time_S, ref_end_time_S,
                            ref_del_time_S)
...
for (RH_list, RH_name) in zip(exp_list, exp_name_list):
    ...
    #               FOOT PRESSURE SENSOR DATA             ########
    ##############################################################
    # exception for stance data
    if (RH_num == "02") | (RH_num == "04"):
        pass
    else:
        # RasPi stance path
        (stance_list, stance_name_list) = folder_path_name(
        ...
        for (folder_list, folder_name) in zip(stance_list, stance_name_list):
            ...
            (L_path, L_name) = folder_path_name(L_csv_path,
                                                "start", "interp", 1)
            for (L_data, L_data_name) in zip(L_path, L_name):
            ...
            (R_path, R_name) = folder_path_name(R_csv_path,
                                                "start", "interp", 1)
            for (R_data, R_data_name) in zip(R_path, R_name):
